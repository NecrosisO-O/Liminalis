...
- The current execution run has now extended the backend foundation through a first minimal `M7` implementation covering `ExtractionAccess`, password challenge state and captcha escalation after one failed password, extraction retrieval-count decrement on successful protected completion, `PublicLink`, short-lived public-link delivery tickets, and public-link download-count decrement on successful ticket redemption.
- The current `M7` validation baseline now includes e2e coverage for extraction password failure escalation, confidential-level system-generated extraction passwords, extraction retrieval-count decrement on successful completion, public-link ticket issuance and redemption, and policy blocking for disabled extraction/public-link modes.
- When new Prisma-backed write-side models are introduced in this repo, local validation currently requires an explicit schema sync against the project database with `prisma db push`; for the `M7` schema expansion, local validation used `--accept-data-loss` only to acknowledge Prisma's warning about adding a new unique index in the local development database.
- The current execution run has now extended the backend foundation through a first minimal `M8` implementation covering invite listing and invalidation, user listing, versioned policy publication, policy history visibility, restore-defaults publication, persisted default confidentiality-level settings, and metadata-only operational summary endpoints.
- The current `M8` validation baseline now includes e2e coverage for invite invalidation, policy version publication, policy-history visibility, restore-defaults version creation, persisted default-level use during upload preparation, and validation rejection for invalid policy combinations while keeping operations output metadata-only.
- Admin policy publication now must create new current `PolicyBundle` versions rather than mutating current rows in place, and the instance default confidentiality level is now persisted separately through `InstanceSetting` so later uploads use the current admin-selected default.
